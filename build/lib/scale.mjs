...
/**
 * 難易度 1 つ分だけを 1 行で説明する。**書籍詳細ページはこちらを使う。**
 *
 * degreeTable() は表と役割の定義で約 1,000 字あり、これを 1,390 枚の書籍ページへ
 * 複製すると、1 ページの本文（約 4,100 字）の四分の一が全ページ共通の同じ文章になる。
 * AdSense のポリシー（複数ページで長文を繰り返し貼り付けない）に触れるため、
 * 2026-09-05 に書籍ページだけ 1 行へ落とし、表の全文は /methodology/ を正本にした。
 * 表を出してよいのは、ページ数が少ない methodology と参考書一覧（7 枚）だけ。
 */
export function degreeLine(d) {
  const l = LEVELS.find(x => x.d === d) || LEVELS[4];
  // **到達目安（hensachi）をここに出さない。** LEVELS が持つのは難易度の帯ごとの
  // 汎用値で、本ごとの到達目安（book.hensachi）とは別物である。表として 10 段階を
  // 並べるなら両者の違いは文脈で読めるが、1 行に切り出すとその本の値に見え、
  // すぐ上の基本情報と食い違う（難易度 5 の本で 48〜60 と 50〜62 が並ぶ）。
  // 本ごとの到達目安は基本情報の欄が正本。ここは帯と定義への導線だけを持つ。
  /* 10 段階の定義と役割との違いの説明は、以前ここに 1 文まるごと置いていた。
     同じ文が 1,390 ページに並び、本文の 1.7% を占めていたのでリンクだけに畳んだ
     （説明の本体は /methodology/ にある）。→ build/lib/compare-note.mjs の経緯 */
  return `<p class="scale__line">難易度 <b>${l.d} / 10</b> は「${esc(l.band)}」の帯です（<a href="/methodology/">10 段階の定義</a>）。</p>`;
}
