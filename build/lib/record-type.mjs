 ...
 *
 * ## 枠にも見本の書影を出す（2026-09-05）
 *
 * 枠だけ書影が無いと、一覧・ルートの中でそこだけ代替表示（書名＋出版社の地色）になり、
 * 並びから浮く。そこで `BOOKS[].coverExample` に**見本**として 1 枚だけ持たせている
 * （赤本の見た目が分かる例として、大学赤本シリーズ「東京大学（理科）」を使う）。
 * **これは表示だけの話で、枠の性質は変えていない。** ISBN も購入リンクも Book JSON-LD も
 * 出さないままで、画像の alt には見本であることを書く。以前の誤りは「枠に東大の赤本の
 * ISBN を入れ、購入リンクと JSON-LD まで東大の赤本を指していた」ことであって、
 * 見本の画像を出すこと自体ではない。
 *
