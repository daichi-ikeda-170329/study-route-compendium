...
ルート上の枠（「志望校の大学別過去問題集」など）は特定の商品ではないので `cover` を
持てないが、そこだけ代替表示になって並びから浮くため、`BOOKS[].coverExample` に
**見本**を 1 枚だけ持たせている（2026-09-05 の判断。詳細は docs/data-model.md の「書影」の節）。
枠の ISBN・購入リンク・構造化データは従来どおり特定の商品を指さず、書籍ページの
`alt` にも見本であることを書く。
...
| `googlebooks` | https://developers.google.com/books/terms | **書影の条件は書かれていない。** 料金・コンテンツ削除・プライバシーを定めるだけで、サムネイル画像の表示可否・帰属表示・API 経由の要否には触れていない。**別途あたる必要がある** |
