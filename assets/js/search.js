 ...
 * ルート大全 — 全ページ共通の参考書検索
 *
 * ヘッダーに常設した検索ボックス（#rtSearch）から、全科目の参考書を横断して
...
(function (global) {
  ...
  var STYLE = [
    ...
    /* align-self:stretch で入力欄を枠の高さ（38px）いっぱいにする。
       付けないと入力欄自体は文字の高さ（20px）しか無く、枠の上下 9px を押しても
       フォーカスが入らない。WCAG 2.5.8 のターゲットサイズ（24px）も割る。
       見た目は変わらない（背景も枠線も持たないため）。2026-09-19 に実測して修正 */
    ".rt-search input{flex:1;align-self:stretch;min-width:0;border:0;outline:0;background:none;font-family:inherit;font-size:13px;color:var(--ink,#22242B);-webkit-appearance:none;appearance:none}",
