...
function renderRouteBody(){
  ...
  const nodes = seq.map((s,i)=>{
    ...
    return `<div class="climb-node${cls}" data-book-id="${b.id}" data-subject-id="math" data-h="${b.h}" data-hours="${String(b.hours||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;")}">
  ...
  const paraHtml = paras.length ? `
    ...
    ${paras.map(p=>{
      ...
      return `<div class="climb-node para" data-book-id="${b.id}" data-subject-id="math">
      ...
      <div class="opt-list" style="margin-top:14px">
        ${books.slice(0,4).map((s,i)=>{
          ...
          return `<button class="opt" onclick="openModal('${b.id}')">
            ...
            <span class="opt__txt"><b>${bookName(b)}</b><span>${st.label} ・ ${b.hours}</span></span>
          </button>`;}).join("")}
