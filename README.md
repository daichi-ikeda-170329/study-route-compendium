...
- [x] リッチリザルト検査で、書籍ページのパンくずと `Book` を確認（**2026-09-19 に実施**。`/science/books/jokyu-seiko/` で「1 件の有効なアイテムを検出しました」＝**パンくずリストは有効・エラー 0**。`Book` は検出されないが、これは正常 — Google のリッチリザルトテストはリッチリザルトの対象になる型だけを検出し、`Book` は対象外。構造化データ自体は生成ページ 1,671 枚を全件パースして検証済み）
- [x] 数値を `docs/kpi-snapshots.md` へ**期間つきで**記録（**2026-09-19 に記録**。Search Console と GA4 の実測。GA4 側は CI の水増しを含むと明記した）
- [x] 実測した件数を `docs/search-console-indexing.md` の「4. 実測の記録」へ 1 行足す（2026-09-07・2026-09-18）

### Google アナリティクス 4

- [x] イベントが `docs/analytics-events.md` の表と一致しているか確認（**2026-09-19 に実測**。allowlist の 19 件のうち**実際に送っているのは 10 件**で、科目トップの計測 9 件は契約だけで未実装だった（実ブラウザで図鑑・ルート・診断を操作しても呼ばれない）。表を「送っている」「まだ呼び出しが無い」に分け、`test/analytics.test.mjs` が食い違いを検出するようにした。**実装は流入が戻ってから**）
...
- [x] 2026-09-11 に運営者の判断で決めた。**プログラムとマークアップは MIT**（`LICENSE`）、
      **文章とデータ（`data/`・`build/content/`・`docs/` の文章、難易度・到達目安・想定学習時間・
      書籍の解説などの評価データ）は著作権を留保**（`data/LICENSE.md`。無断転載・再配布を禁じる）。
      書影は出版社などの著作物で、リポジトリに含めず、サイトでは提供元の商品画像 URL を参照して
      表示している（保存・加工はしていない）。`LICENSE` は公開物（`dist/`）には入れない
