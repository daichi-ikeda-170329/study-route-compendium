...
node .Codex/skills/seo-rank-watch/scripts/status.mjs --repo .

# GSC から順位を取る（認証情報が要る。未設定なら終了コード 2）
node .Codex/skills/seo-rank-watch/scripts/fetch_gsc_ranks.mjs --repo . --append
node .Codex/skills/seo-rank-watch/scripts/fetch_gsc_ranks.mjs --repo . --days 7

# WebSearch で目視した順位を追記する
echo '[{"keyword":"青チャート Focus Gold どっち","rank":3,"url":"https://route-taizen.com/math/guides/ao-vs-fg/"}]' \
  | node .Codex/skills/seo-rank-watch/scripts/record_ranks.mjs --repo . --source websearch
```
